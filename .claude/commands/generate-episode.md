---
description: Genera un nou episodi del Podcast del Doctor (transcripció, capítols, soundbite, thumbnail, archive.org, deploy)
argument-hint: "[fonts opcionals: URLs separades per espais]"
---
...
El contingut es genera amb **intel·ligència artificial** basant-se en fonts públiques, i les transcripcions amb **OpenAI Whisper**.
Els MP3 s'allotgen a **archive.org** (no a GitHub).

**El número d'episodi SEMPRE s'autogenera** (mai es passa com a argument): es deriva al PAS PREVI a partir de l'últim episodi existent.

**Els arguments (`$ARGUMENTS`), si n'hi ha, són SEMPRE fonts** (URLs d'articles, papers, vídeos…). Fes-les servir com a fonts de l'episodi. Tant si en passes com si no, al PAS 0 demana sempre si hi ha fonts addicionals — pots combinar les dels arguments amb les que t'indiqui (qualsevol de les dues vies, o totes dues).

---

## WORKFLOW COMPLET

Segueix aquests passos **en ordre estricte**. No saltis cap pas. Fes servir la llista de tasques (TaskCreate/TaskUpdate) per fer el seguiment.

### PAS PREVI: Preparar directori d'staging

**Fes-ho SEMPRE al principi, abans de demanar res més.**

1. Determinar **sempre automàticament** el número del pròxim episodi (mai a partir d'arguments):
```bash
ls _episodes/ | sort | tail -1
```
Agafar el número de l'últim episodi i sumar 1. Formatar amb zeros: `012`, `013`, etc.
...
6. Un cop confirmat, moure l'MP3 a `episodes/` i les fonts a `sources/`, i **esborrar el directori d'staging** (ja no cal un cop tot copiat):
```bash
mv /tmp/podcast-staging/XXX/*.mp3 episodes/
mv /tmp/podcast-staging/XXX/* sources/ 2>/dev/null || true
rm -rf /tmp/podcast-staging/XXX
```
...
1. **Fitxer MP3**: Confirmar que l'MP3 s'ha mogut a `episodes/` en el pas anterior.
   - Format esperat: `XXX-nom-descriptiu.mp3` (ex: `012-tema-episodi.mp3`)
   - Si no existeix, demanar-lo.

2. **Fonts**: Les fonts poden arribar pels arguments (`$ARGUMENTS`) i/o demanant-les. Si s'han passat URLs com a arguments, fes-les servir com a fonts. **Sempre**, a més, pregunta si hi ha fonts addicionals (i combina-les amb les dels arguments):
   - 📄 URLs de les fonts principals (articles, papers, documentació, etc.)
   - 🎥 URLs de vídeos (si n'hi ha, ex: YouTube)
...
2. Llegir els últims 2-3 episodis a `_episodes/` per buscar **referències creuades** (temes que continuen o es mencionen entre episodis — per exemple la sèrie sobre COLMAP dels episodis 008/009/010).
...
```json
{
  ...
  "chapters": [
    ...
    { "startTime": NN.N, "title": "Tema 1: ..." },
    { "startTime": NN.N, "title": "Tema 2: ..." },
    { "startTime": NN.N, "title": "Reflexió final" }
  ]
}
```
...
- Els capítols del mig corresponen als temes principals de la secció "Temes tractats"
...
⚠️ **IMPORTANT:** El markdown que genera `transcribe_episode.py` al PAS 2 conté la **transcripció sencera al body** i ja inclou un **disclaimer final**. Cal **REESCRIURE el fitxer sencer** (frontmatter + body), no afegir-hi contingut al final — si no, queden disclaimers duplicats (va passar a l'episodi 008) i la transcripció al body. El camp `audio_file` auto-generat conté el nom del fitxer local (ex: `XXX-nom-episodi.mp3`); deixar-lo buit aquí, es completarà al PAS 8.

Reescriure `_episodes/XXX-nom-episodi.md` amb el format definitiu (mira l'episodi 010 o 011 com a referència de format):
...
**Si falla per rate limit:** informar l'usuari que cal esperar 30-60 minuts i tornar a intentar.

Verificar que `audio_file` al markdown s'ha actualitzat correctament (URL completa amb `https://`). Si no, actualitzar-lo manualment.
...
- **Disclaimer**: Sempre incloure que el contingut és generat amb intel·ligència artificial basant-se en fonts públiques
...
- **Disclaimer únic**: Reescriure el fitxer sencer al PAS 6 per evitar disclaimers duplicats (com va passar a l'episodi 008)
